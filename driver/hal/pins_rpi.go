...
// rpi4PinMapper handles pin mapping for Raspberry Pi 4.
// Uses the same mappings as Pi 5.
type rpi4PinMapper struct {
	rpi5PinMapper
}

func (m *rpi4PinMapper) PhysicalToGPIO(physical int) (int, error) {
	if gpio, ok := rpiPhysicalPins[physical]; ok {
		return gpio, nil
	}
	return 0, fmt.Errorf("%w: invalid physical pin %d for Raspberry Pi 4", ErrPinNotFound, physical)
}

func (m *rpi4PinMapper) NameToGPIO(name string) (int, error) {
	gpio, err := m.rpi5PinMapper.NameToGPIO(name)
	if err != nil {
		return 0, fmt.Errorf("%w: unknown pin name %q for Raspberry Pi 4", ErrPinNotFound, name)
	}
	return gpio, nil
}

// rpi3PinMapper handles pin mapping for Raspberry Pi 3.
// Uses the same mappings as Pi 4/5.
type rpi3PinMapper struct {
	rpi5PinMapper
}

func (m *rpi3PinMapper) PhysicalToGPIO(physical int) (int, error) {
	if gpio, ok := rpiPhysicalPins[physical]; ok {
		return gpio, nil
	}
	return 0, fmt.Errorf("%w: invalid physical pin %d for Raspberry Pi 3", ErrPinNotFound, physical)
}

func (m *rpi3PinMapper) NameToGPIO(name string) (int, error) {
	gpio, err := m.rpi5PinMapper.NameToGPIO(name)
	if err != nil {
		return 0, fmt.Errorf("%w: unknown pin name %q for Raspberry Pi 3", ErrPinNotFound, name)
	}
	return gpio, nil
}
